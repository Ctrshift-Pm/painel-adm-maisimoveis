<script lang="ts">
  ...
  async function openDocumentsModal(broker: Broker) {
    selectedBroker = broker;
    selectedDocuments = broker.documents ?? null;
    docsError = null;
    isDocumentsModalOpen = true;

    const token = get(authToken);
    if (!token) {
      docsError = 'Sessão expirada. Faça login novamente.';
      return;
    }
    ...
    try {
      const response = await fetch(`${API_BASE}/brokers/${broker.id}/documents`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      if (response.ok) {
        const payload = await response.json();
        selectedDocuments = {
          creci_front_url: payload?.creci_front_url ?? payload?.documents?.creci_front_url ?? null,
          creci_back_url: payload?.creci_back_url ?? payload?.documents?.creci_back_url ?? null,
          selfie_url: payload?.selfie_url ?? payload?.documents?.selfie_url ?? null
        };
      } else if (response.status === 404) {
        docsError = 'Documentos não encontrados para este corretor.';
      } else {
        const payload = await response.json().catch(() => null);
      ...
      docsError =
        err instanceof Error ? err.message : 'Ocorreu um erro ao carregar os documentos.';
    } finally {
      docsLoading = false;
    }
  }
  ...
  <header class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div>
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400">
        Consulte os corretores cadastrados, aprove ou rejeite solicitações e visualize os
        documentos enviados.
      </p>
    </div>
    <button
    ...
    <div class="flex flex-col items-center justify-center rounded-md border border-dashed border-gray-300 bg-white p-12 text-center dark:border-gray-700 dark:bg-gray-800">
      ...
      >
        <path
          ...
          stroke-linejoin="round"
          stroke-width="2"
          d="M8 16h8m-4-4h4m-5-8a3 3 0 016 0v12a5 5 0 01-5 5H7a5 5 0 01-5-5V7a3 3 0 016 0"
        />
      </svg>
              ...
              <td class="px-6 py-4">
                <div class="flex flex-col items-end gap-2 sm:flex-row">
                  ...
                  <button
                    ...
                    on:click={() => openDocumentsModal(broker)}
                  >
                    <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="2"
                        d="M12 8c1.38 0 2.5-1.12 2.5-2.5S13.38 3 12 3s-2.5 1.12-2.5 2.5S10.62 8 12 8zm0 0c2.21 0 4 1.79 4 4v1a2 2 0 002 2v3h-12v-3a2 2 0 002-2v-1c0-2.21 1.79-4 4-4z"
                      />
                    </svg>
                    Ver documentos
                  </button>
