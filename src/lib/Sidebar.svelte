<script lang="ts">
    ...
    // Corrija a tipagem da função onNavigate
    ...
    function isValidView(view: string): view is View {
        const validViews: View[] = ['dashboard', 'properties', 'property_requests', 'brokers', 'broker_requests', 'clients', 'verification', 'notifications'];
    ...
    function handleNavigation(view: string) {
        if (isValidView(view)) {
            ...
            console.error(`Tentativa de navegar para view inválida: ${view}`);
...
<aside class="fixed inset-y-0 left-0 z-30 w-64 bg-gray-800 dark:bg-gray-900 text-white flex flex-col transform transition-transform duration-300 ease-in-out lg:translate-x-0 {isOpen ? 'translate-x-0' : '-translate-x-full'}">
    ...
    <nav class="flex-1 px-4 py-4 space-y-2">
        ...
        <button class="w-full text-left flex items-center px-4 py-2 rounded-lg transition-colors {activeView === 'properties' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' : 'hover:bg-gray-700'}" on:click={() => handleNavigation('properties')}>
        ...
        <button class="w-full text-left flex items-center px-4 py-2 rounded-lg transition-colors {activeView === 'property_requests' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' : 'hover:bg-gray-700'}" on:click={() => handleNavigation('property_requests')}>
        ...
        <button class="w-full text-left flex items-center px-4 py-2 rounded-lg transition-colors {activeView === 'verification' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' : 'hover:bg-gray-700'}" on:click={() => handleNavigation('verification')}>
            <svg class="w-6 h-6 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
            Solicitações de Verificação
        </button>
        <button class="w-full text-left flex items-center px-4 py-2 rounded-lg transition-colors {activeView === 'notifications' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' : 'hover:bg-gray-700'}" on:click={() => handleNavigation('notifications')}>
            <svg class="w-6 h-6 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8m-18 8h18a2 2 0 002-2V6a2 2 0 00-2-2H3a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
            Notificações
        </button>
    </nav>
